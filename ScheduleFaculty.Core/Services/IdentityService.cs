...
public class IdentityService : IIdentityService
{
    ...
    public async Task<ActionResponse> Login(LoginRequest request)
    {
        ...
        if (!roles.Contains("Secretary") && !roles.Contains("Professor") && !roles.Contains("LabAssistant"))
        {
            return new ActionResponse<string>
            {
                Action = "Login",
                Errors = new List<string> { "You must be a Secretary, Professor or Lab Assistant to login!" }
            };
        }
